...
const ABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      { internalType: "uint8", name: "votesReceived", type: "uint8" },
  ...
  },
  {
    inputs: [
      ...
      { internalType: "enum SimpleVoting.Role", name: "_role", type: "uint8" },
    ],
...
async function toggleVoting() {
  ...
  setState("votingActive", !(await getState("votingActive")));
...
async function toggleResult() {
  ...
  setState("resultsActive", !(await getState("resultsActive")));
  return await Moralis.executeFunction(options);
}
...
async function getListOfCandidates() {
  const options = {
    chain: CHAIN,
    contractAddress: CONTRACTADDRESS,
    functionName: "getListOfCandidates",
    abi: ABI,
  };
  setState("listOfCandidates", await Moralis.executeFunction(options));
  return await Moralis.executeFunction(options);
}

if(getStateData("listOfCandidates")){
document.getElementById("candidateCount").onload = countCandidates;
}

const countCandidates = () => {
  const candidateCount = document.getElementById("candidateCount");
  candidateCount.innerText = getStateData("listOfCandidates").length;
  console.log(getStateData("listOfCandidates").length);
}
